...
const loginUserBank = async (req, res) => {
    ...
    .json({"message": `Bank Account login successfully for account number ${user.account_id}`})
...
const deleteAccount = async(req,res)=>{
    if(!req.params.account_id){
        return res.status(400).send({response:"missing account number id in params"})
    }
    try {
...
try {
      ...
      if(senderUser.balance <=Amount){
       return res.json({msg :` your amount  is bigger then balaence and balaence`})
